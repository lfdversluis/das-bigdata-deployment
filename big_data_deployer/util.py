...
def execute_command_quietly(command_line_list):
    ...
    with open(os.devnull, "wb") as devnull:
        subprocess.check_call(command_line_list, stdout=devnull, stderr=subprocess.STDOUT, shell=True)

def execute_command_for_output(command_line_list):
    return subprocess.Popen(command_line_list, stdout=subprocess.PIPE, shell=True).communicate()[0].decode("utf-8")
